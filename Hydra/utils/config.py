...
def config_reader(theme):
    logger = logging
    logger.basicConfig(filename="Hydra.log", filemode="a", level=logger.INFO)
    json_file = ["config", "config1", "config2"]

    json_file_path = LOCATION + r"{}.json"
    default_path = "../"
    if theme == 0:
        try:
            ...
            logger.info(
                os.getcwd() + " Attempting to correct directory and find config.json"
            )
